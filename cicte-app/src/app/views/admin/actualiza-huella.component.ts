...
@Component({
  selector: 'app-actualiza-huella',
  templateUrl: './actualiza-huella.component.html',
  styleUrls: ['./selecionar-huella.component.scss']
})
export class ActualizaHuellaComponent implements OnInit {
  @Input() itemRes: any;
  ...
  selectFingerToCapture(hand: string, finger: string) {
    ...
    if (this._flag) {
      ...
      alert('El piloto no dispone de registro biométrico');
    }
  }
  ...
  fingerStatus(item: any) {
    ...
    this.services.getFingerStatus(request).subscribe(data => {
      ...
      if (data['success']) {
        ...
        alert('El piloto no cuenta con un registro biométrico');
        this._flag = false;
        this.resetFinger();
      }
    });
  ...
  update(item: any) {
    ...
    this.services.update(item).subscribe(data => {
      if (data['success']) {
        this.alert = {
